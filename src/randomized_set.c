...
RandomizedSet* randomizedSetCreate() {
    RandomizedSet* set = (RandomizedSet*)malloc(sizeof(RandomizedSet)); // Alloue de la mémoire pour la structure RandomizedSet.
    set->nums = (int*)malloc(1000 * sizeof(int)); // Alloue de la mémoire pour le tableau de nombres avec une capacité initiale de 1000.
...
bool randomizedSetInsert(RandomizedSet* obj, int val) {
    ...
    if (obj->numsSize == obj->capacity) { // Vérifie si le tableau est plein.
        obj->nums = (int*)realloc(obj->nums, (2 * obj->capacity) * sizeof(int)); // Réalloue de la mémoire pour doubler la capacité du tableau.
...
bool randomizedSetRemove(RandomizedSet* obj, int val) {
    for (int i = 0; i < obj->numsSize; i++) {
        if (obj->nums[i] == val) { // Recherche l'élément dans le tableau.
            obj->nums[i] = obj->nums[obj->numsSize - 1]; // Remplace l'élément par le dernier élément du tableau pour maintenir la continuité.
